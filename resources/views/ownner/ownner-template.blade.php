      ...
      <ul class="app-nav">
        ...
        <li class="dropdown"><a class="app-nav__item" href="#" data-toggle="dropdown" aria-label="Open Profile Menu"><i class="fa fa-user fa-lg"></i></a>
          <ul class="dropdown-menu settings-menu dropdown-menu-right">
            ...
            <li><a class="dropdown-item" href="{{ route('ownner.logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();"><i class="fa fa-sign-out fa-lg"></i>  {{ __('Logout') }}</a></li>
            <form id="logout-form" action="{{ route('ownner.logout') }}" method="POST">
                    {{ csrf_field() }}
            </form>
          </ul>
        </li>
    ...
    <aside class="app-sidebar" style="background-color: {{env('THEME_COLOR')}}">
      <div class="app-sidebar__user"><img class="app-sidebar__user-avatar" src="{{asset(env('APP_LOGO'))}}" alt="User Image" style="max-height: 64px; max-width: 64px">
        <div>
          ...
          <p class="app-sidebar__user-name"><small>ownner</small></p>
        </div>
      </div>
      <ul class="app-menu">

        <li><a class="app-menu__item {{(Request::is('ownner'))? 'active': ''}}" href="{{route('ownner.home')}}"><i class="app-menu__icon fa fa-dashboard"></i><span class="app-menu__label">Dashboard</span></a></li>
